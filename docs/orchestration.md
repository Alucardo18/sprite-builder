...
| Creative Generation | Request + referencias | Una hoja PNG nativa por dirección/candidato |
...
2. Codex `image_gen` genera una hoja por request por defecto; `generate-openai` es la ruta API
   opcional y explícita para GPT Image 2. Ambas conservan los bytes de la hoja.
...
La generación en Codex usa `image_gen` por defecto. La ruta API GPT Image 2 requiere
`OPENAI_API_KEY` y solo se usa mediante `generate-openai`; nunca se improvisan llamadas HTTP.
