...
- El editor de **Fondo & Estudio** (Tab 1) ofrece la suite completa de herramientas de retoque pixel-art directamente sobre el sprite sheet: varita (`wand`), borrador (`eraser`), lápiz (`pencil`), cuentagotas (`eyedropper`), cubeta/relleno (`fill`), reemplazar color (`replace_color`), recorte (`crop_lasso`, `crop_rect`, `crop_ellipse`), selección (`select_lasso`, `select_rect`, `select_ellipse`), mover selección flotante (`move`), rotación libre y acciones de píxeles (voltear H/V, rotar 90°/180°, escalar 2×/½, contorno de 1 px y limpieza de píxeles aislados), además de paleta y selector de color. El resize por esquina cuantiza siempre a pasos enteros de 1 px; el imán del grid se limita a una captura de 1 px y Alt/Option lo desactiva durante el drag.
- Una selección flotante puede usar **Encajar al grid** desde la barra o con clic derecho sobre la pieza. La herramienta mide el cuerpo visual denso de la imagen —no el margen del lazo ni píxeles alfa residuales aislados—, propone el bloque más cercano del grid activo, permite margen y anclaje, y solo reduce con nearest-neighbor cuando el usuario activa esa opción. El menú de clic derecho añade **Ajustar al bloque del grid**, una acción directa de fine-tuning que ajusta X/Y con nearest-neighbor hasta rellenar exactamente los cuatro bordes del bloque más cercano, aunque requiera una variación mínima de proporción. La selección permanece flotante después de aplicar cualquiera de las dos acciones para poder seguir moviéndola.
- El mismo menú contextual permite posicionar sin escalar, encajar conservando proporción, alinear con nueve anclajes dentro del bloque, recortar la máscara al contenido visible y duplicar la pieza hacia cualquiera de las cuatro celdas vecinas. La **Vista repetida 3×3** es una previsualización local no destructiva: no emite operaciones, no modifica el PNG y no llega a exportación.
